...
def inference(data, model, tokenizer, device='cpu'):
    ...
    eval_dataloader = DataLoader(data, sampler=eval_sampler,
                                 batch_size=len(data))
    model.eval()
    p = []
...
def get_features(examples, tokenizer, max_code_len):
    features = convert_examples_to_features(examples, tokenizer,
                                            stage='test')
    all_source_ids = torch.tensor([f.source_ids[: max_code_len]
                                   for f in features], dtype=torch.long)
    all_source_mask = torch.tensor([f.source_mask[: max_code_len]
                                    for f in features], dtype=torch.long)
    return TensorDataset(all_source_ids, all_source_mask)


def build_model(model_class, model_file, config, tokenizer, max_len,
                beam_size, device='cpu'):
    ...
    model = Seq2Seq(encoder=encoder, decoder=decoder, config=config,
                    beam_size=beam_size, max_length=max_len,
